...
*The official ADEWALE CLASSROOM logo and founder photo are embedded on the landing page (assets/).*
...
| 4 | **🎬 Per-teacher recording brands** | Recordings now carry **the recording teacher's own brand, not HMG's**: the setup dialog asks for *Brand name* (prefills from their account), *Footer credit* (their YouTube/WhatsApp line) and an optional **logo upload** (auto-downscaled and stored on their device). The video header shows their logo — or a clean coloured initial badge if none — with their brand on both sides; the footer carries their credit line (defaulting to a subtle “Recorded with ADEWALE CLASSROOM DECK”); the filename starts with their brand (`Funkes-Tutorials_Maths_…webm`). Every teacher-client gets YouTube-ready videos under their own identity — a real selling point for your subscriptions. |
...
| **🎨 White-label branding** | Settings → set your **academy name** (replaces the broadcast watermark, e.g. "ADEWALE CLASSROOM") and pick an **accent colour** — rebrand without touching code. Useful when other teachers/franchises use your deployment. |
...
git init
git add .
git commit -m "ADEWALE CLASSROOM DECK v1.0"
git branch -M main
...
Built with ❤ for **ADEWALE CLASSROOM** · [cssadewale.pages.dev](https://cssadewale.pages.dev) ·
[hmgconcepts.pages.dev](https://hmgconcepts.pages.dev) · [adewaleclassroom.pages.dev](https://adewaleclassroom.pages.dev) ·
