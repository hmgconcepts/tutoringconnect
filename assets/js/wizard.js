/* ====================================================================
   wizard.js — Tutoring Connect Gen v7
...
const Wizard = {
  ...
  init() {
    ...
    try {
      ...
      if (preset && window.TC && window.TC.PRESETS && window.TC.PRESETS[preset]) {
  ...
  async previewLive() {
    ...
    const theme = window.TC.THEMES.find(t => t.id === cfg.themeId) || window.TC.THEMES[0];
    cfg.themePrimary = theme.primary;
    cfg.themeAccent = theme.accent;
    const font = window.TC.FONTS.find(f => f.id === cfg.fontId) || window.TC.FONTS[0];
  ...
  async fullPreview() {
    ...
    if (!cfg.modules || !cfg.modules.length) {
      cfg.modules = window.TC.PRESETS.secondary.modules.slice();
      cfg.levels = window.TC.PRESETS.secondary.levels.slice();
    }
...
console.log('%c[Tutoring Connect Gen v7] wizard ready.', 'color:#4f46e5');
